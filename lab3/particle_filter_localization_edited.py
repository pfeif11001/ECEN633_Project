...
class ParticleFilterLocalizer():
    ...
    def resample_particles(self, chanceOfRandom):
        """Resample particles in place according to the probabilities in self.weights"""
        ####################################
        # Finish this implemenation!!
        #print("change of rand")
        #print(chanceOfRandom)
        numRandomSamples = int(self.N * chanceOfRandom)
        numNormalSamples = self.N - numRandomSamples
        #print("num norm samples")
        #print(numNormalSamples)
        indexesToSample = np.random.choice(self.N, numNormalSamples, p=self.weights)
        self.particles = self.particles[indexesToSample]
        randParticles = np.random.uniform([self.xlim[0], self.ylim[0], 0],
                                        [self.xlim[1], self.ylim[1], 360], (numRandomSamples,3))
        self.particles = np.append(self.particles, randParticles, axis=0)
    ...
    def iterate(self, u: 'np.ndarray[(3,) , np.dtype[np.float64]]',
        ...
        wAvg = np.mean(self.weights)
        #print('wAvg')
        #print(wAvg)
        self.W_SLOW += ALPHA_SLOW*(wAvg - self.W_SLOW)
        self.W_FAST += ALPHA_FAST*(wAvg - self.W_FAST)
        self.W_SLOW_HIST.append(self.W_SLOW)
        self.W_FAST_HIST.append(self.W_FAST)
        #print("wFast")
        #print(self.W_FAST)
        self.normalize_weights()
        
        chanceOfRandom = np.clip(1.0 - self.W_FAST/self.W_SLOW, 0, 1)
        
        # Resample particles
        self.resample_particles(chanceOfRandom)
...
def main(plot_live: bool, mapfile: str, datafile: str, num: int, makeGif: bool):
    ...
    pf = ParticleFilterLocalizer(prior_map, [15, 80], [15, 80], num, alphas, cov)    
    ...
    plt.plot(pf.W_SLOW_HIST, color="Green")
    plt.plot(pf.W_FAST_HIST, color="Red")
    plt.show()

...
class OccupancyGrid:
    ...
    def __init__(self, map_size, config):
        ...
        self.log_prob_map = np.zeros([map_size[0] * self.resolution, map_size[1] * self.resolution], dtype=float)
    ...
    def save_to_csv(self, file_path):
        """
        Saves the occupancy grid to a CSV file with 1 for occupied and 0 for unoccupied.
        
        :param file_path: The path to the CSV file to be saved.
        """
        prob_map = self.fetch_prob_map()
        # Convert probabilities to binary values
        binary_map = np.zeros(prob_map.shape, dtype=int)
        for i in range(prob_map.shape[0]):
            for j in range(prob_map.shape[1]):
                binary_map[i][j] = 0 if prob_map[i][j] > 0.5 else 1
        
        # Transpose the binary_map to avoid rotation
        binary_map = binary_map.T
        
        np.savetxt(file_path, binary_map, fmt='%d', delimiter=',')
